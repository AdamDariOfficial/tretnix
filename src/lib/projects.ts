...
export async function listVisibleProjects(): Promise<Project[]> {
  return listPublicProjects({ data: {} }) as Promise<Project[]>;
}

export async function listFeaturedProjects(limit = 2): Promise<Project[]> {
  const rows = (await listPublicProjects({ data: { featured: true } })) as Project[];
  return rows.slice(0, limit);
}

export async function getProjectBySlug(slug: string): Promise<Project | null> {
  return getPublicProjectBySlug({ data: { slug } }) as Promise<Project | null>;
}

export async function adminListProjects(): Promise<Project[]> {
  return listAdminProjects() as Promise<Project[]>;
}

export async function adminGetProject(id: string): Promise<Project | null> {
  return getAdminProject({ data: { id } }) as Promise<Project | null>;
}

export async function adminUpsertProject(
  project: Partial<Project> & { slug: string; title: string },
): Promise<Project> {
  return saveAdminProject({
    data: { project, csrfToken: await requireAdminCsrfToken() },
  }) as Promise<Project>;
}

export async function adminDeleteProject(id: string) {
  await deleteAdminProject({ data: { id, csrfToken: await requireAdminCsrfToken() } });
}

export async function adminSetVisibility(id: string, is_visible: boolean) {
  await patchAdminProject({
    data: { id, is_visible, csrfToken: await requireAdminCsrfToken() },
  });
}

export async function adminSetFeatured(id: string, is_featured: boolean) {
  await patchAdminProject({
    data: { id, is_featured, csrfToken: await requireAdminCsrfToken() },
  });
}

export async function adminUpdateOrder(id: string, sort_order: number) {
  await patchAdminProject({
    data: { id, sort_order, csrfToken: await requireAdminCsrfToken() },
  });
}
